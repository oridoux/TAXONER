...
if __name__ == "__main__":
    ...
    results = f"# {li}\nExperience launched on {daytime} with with classifier {args.Classifier} and mode {args.mode}\n{li}\n"
    nfps = nfns = ntps = 0
    ...
    for entry in entries:
	    ...
	    os.path.join(args.Corpus, entry), expected[vol_num], args.Classifier, args.mode, args.regex, stopwords)
	    results += result
	    ntps += tps
	    nfps += fps
	    nfns += fns
